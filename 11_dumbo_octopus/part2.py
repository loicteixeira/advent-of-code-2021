...
def find_synchronizing_step(data):
    ...
    for step in count(1):
        ...
        if len(step_flashes) == width * height:
            return step
...
if __name__ == "__main__":
    ...
    expected_result = 195
    assert find_synchronizing_step(example_data) == expected_result

    input_file_path = Path(__file__).parent / "input.txt"
    input_data = input_file_path.read_text().splitlines()

    result = find_synchronizing_step(input_data)
    print(f"{result=}")
